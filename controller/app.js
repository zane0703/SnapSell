            ...
            user.updateUser(
              ...
              function (err, result) {
                if (err) {
                  ...
                  writeFileSync(
                    ".\\public\\image\\profile\\" + userid + ".jpg",
                    req.file.buffer,
                  );
                  if (result.token) {
                    res.status(200);
    ...
    if (err) {
      ...
      verifyToken(req, res, function () {
        if (req.file && req.body.upload == 1) {
          ...
          listings.addListings(
            ...
            function (err, result) {
              if (err) {
                ...
                writeFileSync(
                  ".\\public\\image\\listImg\\" + result.insertId + ".jpg",
                  req.file.buffer,
                );
                res.status(201);
                res.type("json");
          ...
          let { title, description, price, fk_poster_id, picture_url } =
            req.body;
          listings.addListings(
            title,
            description,
